...
1.  `24b0bd9` - **feat: implemetación del backend 2.0**: Integración final de servicios y repositorios.
2.  `616de1a` - **fix: connect frontend to backend**: Corrección de discrepancias en nombres de campos y estados.
3.  `7264d9f` - **test: update tests to match Uteblo brand**: Actualización de pruebas unitarias para reflejar el cambio de marca.
4.  `9790145` - **feat: rename system to Uteblo in sidebar**: Cambio de branding en la interfaz de usuario.
5.  `acac4bd` - **style: rewrite index.css with warm library glassmorphism**: Rediseño visual premium con estética de biblioteca moderna.
6.  `acac4bd` - **style: fix badge colors to warm palette**: Ajuste de colores para coherencia visual.
...
3.  **frontend (React/Vite):** Interfaz de usuario servida en el puerto 5173.
